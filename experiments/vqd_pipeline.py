...
class VQDPipeline:
    ...
    def load_and_split_data(self, data_path=None, sequences=None, labels=None):
        """Load data and create train/test split with REAL labels from MSR dataset."""
        print(f"\n{'='*80}")
        print(f"DATA LOADING & SPLITTING")
        print(f"{'='*80}")
        
        # Load real sequences and labels from MSR Action3D dataset
        if sequences is None or labels is None:
            print("Loading MSR Action3D from raw data files...")
            sys.path.insert(0, str(Path(__file__).parent / 'archive' / 'src'))
            from loader import load_all_sequences
            sequences, labels = load_all_sequences("msr_action_data")
            print(f"✅ Loaded {len(sequences)} sequences from msr_action_data/")
        
        # Build frame bank by sampling representative frames from each sequence
        print("Building frame bank (1 frame per sequence)...")
        frames = []
        for seq in sequences:
            # Take middle frame as representative
            mid_idx = len(seq) // 2
            frames.append(seq[mid_idx])
        
        X = np.vstack(frames)
        y = np.array(labels)
        
        print(f"Dataset: {X.shape}")
        print(f"Classes: {len(np.unique(y))} (range: {y.min()}-{y.max()})")
        print(f"⚠️  Note: Using REAL MSR Action3D labels (not fake generated labels)")
        ...
        print(f"Train: {self.n_train} samples (classes: {len(np.unique(self.y_train))})")
        print(f"Test: {self.n_test} samples (classes: {len(np.unique(self.y_test))})")
...
def main():
    ...
    # Run pipeline with REAL MSR Action3D labels
    (pipeline
     .load_and_split_data()  # No data_path - loads from msr_action_data/
     .preprocess()
     .build_frame_bank(pre_k=8)
     .evaluate_baseline()
     .evaluate_k_sweep()
     .save_results('results/vqd_pipeline_results_CORRECTED.json')
